    ...
    <main style={{ "--scroll-progress": progress } as CSSProperties}>
      ...
      <section className="hero hero-immersive" id="inicio">
        ...
        <div className="transformer-stage" aria-hidden="true">
          <img className="transformer transformer-left" style={{ transform: `translate3d(${(sceneProgress - 1) * 45}vw, ${sceneProgress * 48}px, 0) rotate(${-7 + sceneProgress * 5}deg)` }} src={asset("/assets/transformer-left.png")} alt="" />
          <img className="transformer transformer-right" style={{ transform: `translate3d(${(1 - sceneProgress) * 45}vw, ${sceneProgress * 40}px, 0) rotate(${7 - sceneProgress * 5}deg)` }} src={asset("/assets/transformer-right.png")} alt="" />
        ...
        <div className="hero-logo-wrap" style={{ transform: `translate3d(0, ${-sceneProgress * 170}px, 0) scale(${1 - sceneProgress * .25}) rotate(${sceneProgress * 10}deg)` }} aria-label="Shelby Eletromecânica">
          ...
          <div className="logo-aura" /><img className="hero-logo-preserved" src={asset("/assets/shelby-logo.png")} alt="Logo Shelby Eletromecânica" />
      ...
      <section className="services section" id="servicos">
        ...
        <div className="plate-yard">{services.map((service) => <a href="#diagnostico" className="warning-plate reveal" key={service.title} onClick={() => update("service", service.title)}><div className="plate-inner"><div className="plate-face plate-front"><i>{service.icon}</i><span>ÁREA TÉCNICA</span><h3>{service.title}</h3><b>VIRAR PLACA ↻</b></div><div className="plate-face plate-back"><span>{service.tag}</span><p>{service.copy}</p><b>Solicitar avaliação ↗</b></div></div></a>)}</div>
      </section>
      ...
      <section className="method section" id="metodo">
        ...
        <div className="process-list">{process.map(([title, copy]) => <article className="process-item reveal" key={title}><span>◈</span><div><h3>{title}</h3><p>{copy}</p></div><i>↗</i></article>)}</div>
      </section>
      ...
      <section className="field-gallery section" id="obras"><div className="gallery-head reveal"><div className="section-index">OBRAS EM CAMPO</div><h2>Espaço para<br /><em>mostrar a operação.</em></h2><p>Aqui entram as fotos reais das obras, equipe, equipamentos e antes/depois enviados pela Shelby.</p></div><div className="photo-wall reveal"><div className="photo-slot slot-wide"><span>FOTO DE OBRA EM DESTAQUE</span><i>INSERIR FOTO REAL</i></div><div className="photo-slot"><span>EQUIPE EM CAMPO</span><i>INSERIR FOTO REAL</i></div><div className="photo-slot"><span>SUBESTAÇÃO</span><i>INSERIR FOTO REAL</i></div><div className="photo-slot"><span>INSPEÇÃO TÉCNICA</span><i>INSERIR FOTO REAL</i></div><div className="photo-slot slot-tall"><span>MANUTENÇÃO</span><i>INSERIR FOTO REAL</i></div></div></section>
      ...
      <section className="diagnostic" id="diagnostico">
        <div className="diagnostic-head reveal"><div className="section-index">DIAGNÓSTICO TÉCNICO</div><h2>Conte o cenário.<br /><em>A gente acelera o caminho.</em></h2><p>Em poucos passos, sua solicitação chega à equipe com contexto suficiente para direcionar o atendimento.</p></div>
        <form className="quote-form reveal" onSubmit={submitBrief}>
          ...
          {step === 1 && <fieldset><legend>Qual serviço você precisa?</legend><div className="option-grid">{services.map((service) => <button type="button" className={brief.service === service.title ? "selected" : ""} onClick={() => update("service", service.title)} key={service.title}>{service.title}</button>)}</div><button type="button" className="form-next" onClick={next} disabled={!brief.service}>Continuar <b>→</b></button></fieldset>}
      ...
      <section className="emergency"><div className="emergency-flare" /><div className="section-index">ATENDIMENTO EMERGENCIAL</div><h2>Seu sistema não pode parar.<br /><em>Nós também não.</em></h2><p>Fale direto com nossa equipe técnica.</p><a href="tel:+5511989283393" className="emergency-phone"><small>11</small> 98928-3393 <span>↗</span></a></section>

      <footer id="contato"><div className="footer-main"><div className="footer-brand"><img src={asset("/assets/shelby-logo.png")} alt="Shelby Eletromecânica" /><p>Energia com segurança,<br />confiabilidade e excelência técnica.</p></div><div className="footer-contact"><span>FALE COM A SHELBY</span><a href="#diagnostico">Solicitar diagnóstico</a><a href="mailto:josuel@shelbyeletromecanica.com.br">josuel@shelbyeletromecanica.com.br</a></div><div className="footer-address"><span>BASE OPERACIONAL</span><p>Av. Aparecida, 465<br />Caieiras — SP</p><a href="#inicio">Voltar ao topo ↑</a></div></div><div className="footer-bottom"><span>SHELBY ELETROMECÂNICA</span><span>ENGENHARIA ELÉTRICA • ELETROMECÂNICA • MÉDIA TENSÃO</span></div></footer>
